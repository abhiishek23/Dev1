...
const CodingPage = () => {
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      try {
        const response = await axios.get(`http://51.21.190.160:4000/api/problem/${problemID}`);
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await axios.post("http://51.21.190.160:7000/submit-contest", {
...
const handleRun = async () => {
  ...
  try {
    ...
    const res = await axios.post("http://51.21.190.160:7000/run", {
      language: languageMap[language],
      code,
      input: "",
      userId,
  ...
  const handleAIReview = async () => {
    try {
      setLoadingAction("ai");
      const res = await axios.post("http://51.21.190.160:4000/ai", { code });
 ...
 <div className="p-6 overflow-y-auto bg-black-800 text-white leading-relaxed space-y-4">
  ...
  onSubmitApproach={async (approachText) => {
    ...
    try {
      ...
      const response = await axios.post("http://51.21.190.160:4000/review-approach", {
